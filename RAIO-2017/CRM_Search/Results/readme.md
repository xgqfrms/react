...
if(ts.schema !== undefined && ts.schema.Properties !== undefined){
    ...
    let tabs_length = Object.keys(tabs_obj).length;
    // for of
    let tabs_keys = Object.keys(tabs_obj);
    // ["BasicInformationRow", "AnyManagedFundsRow", "HistoricalReturnsRow", "RiskAnalysisRow ", "lineChartRow ", "lineChartRows "]
    tabs_keys.map(
        (key, index) => {
            let temp_obj = {
                col_name: "",
